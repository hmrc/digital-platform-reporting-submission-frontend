...
class DeleteAssumedReportFailure extends SpecBase {
  ...
  "DeleteAssumedReportFailure" - {
    "must contain correct message" in {
      val test = AssumedReportingConnector.DeleteAssumedReportFailure(status)
      test.getMessage mustBe s"Unexpected status code $status received when trying to delete an assumed report"
    }
  }
}
