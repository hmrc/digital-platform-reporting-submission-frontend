...
class CheckReportingNotificationsControllerSpec extends SpecBase with SummaryListFluency with MockitoSugar with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    Mockito.reset(mockPlatformOperatorConnector, mockSubmissionConnector, mockRecentSubmissionsService)
    super.beforeEach()
  }
  ...
  "Check Reporting Notifications Controller" - {
    ...
    "onSubmit(...)" - {
      ...
      "must redirect to Check Platform Operator when 'true' is selected and business details have not been confirmed" in {
        ...
        when(mockRecentSubmissionsService.confirmReportingNotificationsFor(any())(any()))
        ...
        verify(mockRecentSubmissionsService, times(1)).confirmReportingNotificationsFor(eqTo(operatorId))(any())
