...
import services.RecentSubmissionsService
...
class CheckContactDetailsControllerSpec extends SpecBase with SummaryListFluency with MockitoSugar with BeforeAndAfterEach {
  ...
  private val mockRecentSubmissionsService = mock[RecentSubmissionsService]
  ...
  override def beforeEach(): Unit = {
    Mockito.reset(mockSubscriptionConnector, mockRepository, mockSubmissionConnector, mockRecentSubmissionsService)
    super.beforeEach()
  }
    ...
    "onSubmit(...)" - {
      ...
      "must redirect to manage home page when `false` is submitted" in {
        val application = applicationBuilder(userAnswers = Some(baseAnswers)).overrides(
          ...
          bind[RecentSubmissionsService].toInstance(mockRecentSubmissionsService)
        ).build()
        ...
        verify(mockRecentSubmissionsService, never()).getConfirmedDetailsFor(any())(any())
      }
      ...
      "when `true` is submitted" - {
        "must redirect to Check Platform Operator when business details have not been confirmed" in {
          ...
          when(mockRecentSubmissionsService.confirmYourContactDetailsFor(any())(any()))
          ...
          verify(mockRecentSubmissionsService, times(1)).confirmYourContactDetailsFor(eqTo("some-operator-id"))(any())
          verify(mockSubmissionConnector, never()).start(any(), any(), any())(using any())
        }
