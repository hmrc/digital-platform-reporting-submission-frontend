...
import pages.assumed.update.{AssumingOperatorNamePage, ChrnPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers.*
import repositories.SessionRepository
import views.html.assumed.update.ChrnView
...
class ChrnControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val chrnRoute = routes.ChrnController.onPageLoad(operatorId, caseId).url
  ...
  "Chrn Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("X00000"), operatorId, caseId, assumingOperatorName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual ChrnPage.nextPage(caseId, answers).url
      }
    }
