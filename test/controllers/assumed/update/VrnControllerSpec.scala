...
class VrnControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val formProvider = new VrnFormProvider()
  ...
  lazy val vrnRoute = routes.VrnController.onPageLoad(operatorId, caseId).url
  ...
  "Vrn Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("GB123456789"), operatorId, caseId, assumingOperatorName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual VrnPage.nextPage(caseId, answers).url
      }
    }
