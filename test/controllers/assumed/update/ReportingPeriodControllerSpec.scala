...
import views.html.assumed.update.ReportingPeriodView
...
class ReportingPeriodControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val reportingPeriodRoute = routes.ReportingPeriodController.onPageLoad(operatorId, caseId).url
  ...
  "ReportingPeriod Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(form, operatorId, caseId)(request, messages(application)).toString
      }
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(validAnswer), operatorId, caseId)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual ReportingPeriodPage.nextPage(caseId, answers).url
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, operatorId, caseId)(request, messages(application)).toString
      }
    }
