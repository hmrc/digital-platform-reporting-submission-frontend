...
import pages.assumed.update.{AssumingOperatorNamePage, UkTaxIdentifiersPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers.*
import repositories.SessionRepository
import views.html.assumed.update.UkTaxIdentifiersView
...
class UkTaxIdentifiersControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val ukTaxIdentifiersRoute = routes.UkTaxIdentifiersController.onPageLoad(operatorId, caseId).url
  ...
  "UkTaxIdentifiers Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(UkTaxIdentifiers.values.toSet), operatorId, caseId, assumingOperatorName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val answers = baseAnswers.set(UkTaxIdentifiersPage, Set[UkTaxIdentifiers](UkTaxIdentifiers.values.head)).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual UkTaxIdentifiersPage.nextPage(caseId, answers).url
      }
    }
