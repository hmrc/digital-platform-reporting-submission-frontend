...
import pages.assumed.update.{EmprefPage, AssumingOperatorNamePage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers.*
import repositories.SessionRepository
import views.html.assumed.update.EmprefView
...
class EmprefControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val emprefRoute = routes.EmprefController.onPageLoad(operatorId, caseId).url
  ...
  "Empref Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("123/AB456"), operatorId, caseId, assumingOperatorName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual EmprefPage.nextPage(caseId, answers).url
      }
    }
