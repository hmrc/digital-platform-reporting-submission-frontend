...
import views.html.assumed.update.InternationalAddressView
...
class InternationalAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val internationalAddressRoute = routes.InternationalAddressController.onPageLoad(operatorId, caseId).url
  ...
  "InternationalAddress Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual InternationalAddressPage.nextPage(caseId, userAnswers).url
      }
    }
