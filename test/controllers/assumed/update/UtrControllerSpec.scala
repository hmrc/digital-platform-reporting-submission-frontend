...
import pages.assumed.update.{AssumingOperatorNamePage, UtrPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers.*
import repositories.SessionRepository
import views.html.assumed.update.UtrView
...
class UtrControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val utrRoute = routes.UtrController.onPageLoad(operatorId, caseId).url
  ...
  "Utr Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("1234567890"), operatorId, caseId, assumingOperatorName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual UtrPage.nextPage(caseId, answers).url
      }
    }
