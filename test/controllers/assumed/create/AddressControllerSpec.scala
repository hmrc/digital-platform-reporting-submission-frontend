...
import views.html.assumed.create.AddressView
...
class AddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val addressRoute = routes.AddressController.onPageLoad(NormalMode, operatorId).url
  ...
  "Address Controller" - {
    ...
    "must redirect to Submissions Disabled for a GET when submissions are disabled" in {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual AddressPage.nextPage(NormalMode, userAnswers).url
      }
    }
    ...
    "must redirect to SubmissionsDisabled for a POST if submissions are disabled" in {
