...
import views.html.assumed.create.ReportingNotificationRequiredView
...
class ReportingNotificationRequiredControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  "Reporting Notification Required Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.ReportingNotificationRequiredController.onPageLoad(operatorId).url)
