...
class CreateAssumedReportingSubmissionRequestSpec extends AnyFreeSpec with Matchers
  with TryValues
  with OptionValues
  with EitherValues {

  private val underTest = CreateAssumedReportingSubmissionRequest
  ...
  "build" - {
    "must return correct CreateAssumedReportingSubmissionRequest when relevant data exists" in {
      val answers = aUserAnswers
      ...
      underTest.build(answers) mustBe Right(CreateAssumedReportingSubmissionRequest(
  ...
  ".asAssumedReportingSubmissionRequest" - {
    "must convert CreateAssumedReportingSubmissionRequest to AssumedReportingSubmissionRequest object" in {
      val underTest = CreateAssumedReportingSubmissionRequest(
