    ...
    "in Normal Mode" - {
      ...
      "must go to EMPREF if EMPREF was selected" in {
        ...
        forAll(identifierGen) { identifiers =>
          ...
          VrnPage.nextPage(NormalMode, answers) mustEqual routes.EmprefController.onPageLoad(NormalMode, "operatorId")
        }
      }
      ...
      "must go to CHRN if CHRN was selected and EMPREF was not selected" in {
        ...
        VrnPage.nextPage(NormalMode, answers) mustEqual routes.ChrnController.onPageLoad(NormalMode, "operatorId")
      }
      ...
      "must go to Registered in UK when no later tax identifiers were selected" in {
        ...
        VrnPage.nextPage(NormalMode, answers) mustEqual routes.RegisteredInUkController.onPageLoad(NormalMode, "operatorId")
      }
    }
    ...
    "in Check Mode" - {
      ...
      "must go to EMPREF when EMPREF is selected and has not been answered" in {
        ...
        forAll(identifierGen) { identifiers =>
          ...
          val answers = emptyAnswers.set(UkTaxIdentifiersPage, identifiers).success.value
          VrnPage.nextPage(CheckMode, answers) mustEqual routes.EmprefController.onPageLoad(CheckMode, "operatorId")
        }
      }
      ...
      "must go to CHRN when CHRN is selected and has not been answered" - {
        ...
        "and EMPREF is not selected" in {
          ...
          val answers = emptyAnswers.set(UkTaxIdentifiersPage, Set[UkTaxIdentifiers](Utr, Crn, Vrn, Chrn)).success.value
          VrnPage.nextPage(CheckMode, answers) mustEqual routes.ChrnController.onPageLoad(CheckMode, "operatorId")
        }
      ...
      "must go to Check Answers" - {
        ...
        "when all selected options have been answered" in {
          ...
          forAll(identifierGen) { identifiers =>
            ...
            VrnPage.nextPage(CheckMode, answers) mustEqual routes.CheckYourAnswersController.onPageLoad("operatorId")
          }
        }
      }
    }
