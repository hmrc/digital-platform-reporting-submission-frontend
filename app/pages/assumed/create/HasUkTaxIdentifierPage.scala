...
case object HasUkTaxIdentifierPage extends AssumedReportingQuestionPage[Boolean] {
  ...
  override protected def nextPageNormalMode(answers: UserAnswers): Call =
    answers.get(this).map {
      case true => routes.UkTaxIdentifierController.onPageLoad(NormalMode, answers.operatorId)
    ...
    answers.get(this).map {
      case true =>
        answers.get(UkTaxIdentifierPage)
          ...
          .getOrElse(routes.UkTaxIdentifierController.onPageLoad(CheckMode, answers.operatorId))
