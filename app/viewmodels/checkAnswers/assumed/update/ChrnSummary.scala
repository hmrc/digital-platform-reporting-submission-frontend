  ...
  def row(caseId: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    for {
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.ChrnController.onPageLoad(answers.operatorId, caseId).url)
            .withVisuallyHiddenText(messages("chrn.change.hidden", assumingOperatorName))
        )
      )
    }
}
