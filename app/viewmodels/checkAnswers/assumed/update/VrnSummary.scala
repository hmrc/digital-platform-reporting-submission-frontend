  ...
  def row(caseId: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    for {
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.VrnController.onPageLoad(answers.operatorId, caseId).url)
            .withVisuallyHiddenText(messages("vrn.change.hidden", assumingOperatorName))
        )
      )
    }
}
