  ...
  def row(caseId: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    for {
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.EmprefController.onPageLoad(answers.operatorId, caseId).url)
            .withVisuallyHiddenText(messages("empref.change.hidden", assumingOperatorName))
        )
      )
    }
}
