...
import pages.assumed.update.{AssumingOperatorNamePage, CrnPage}
  ...
  def row(caseId: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    for {
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.CrnController.onPageLoad(answers.operatorId, caseId).url)
