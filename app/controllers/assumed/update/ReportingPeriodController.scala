...
class ReportingPeriodController @Inject()(
  ...
  def onPageLoad(operatorId: String, caseId: String): Action[AnyContent] =
    (identify andThen getData(operatorId, Some(caseId)) andThen requireData) { implicit request =>
      ...
      Ok(view(preparedForm, operatorId, caseId))
    }
  ...
  def onSubmit(operatorId: String, caseId: String): Action[AnyContent] =
    (identify andThen getData(operatorId, Some(caseId)) andThen requireData).async { implicit request =>
      ...
      form.bindFromRequest().fold(
        formWithErrors =>
          Future.successful(BadRequest(view(formWithErrors, operatorId, caseId))),
  
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ReportingPeriodPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(ReportingPeriodPage.nextPage(caseId, updatedAnswers))
      )
    }
}
