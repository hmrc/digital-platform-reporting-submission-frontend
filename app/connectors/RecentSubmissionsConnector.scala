...
class RecentSubmissionsConnector @Inject()(appConfig: FrontendAppConfig, httpClient: HttpClientV2)
  ...
  def save(request: RecentSubmissionDetails)(implicit hc: HeaderCarrier): Future[Done] = {
    httpClient.post(url"${appConfig.digitalPlatformReportingUrl}/digital-platform-reporting/recent-submissions")
      ...
      .flatMap { response =>
        response.status match {
          ...
          case status => Future.failed(SaveRecentSubmissionFailure(status))
        }
      }
  }

  def getRecentUserSubmissions()(implicit hc: HeaderCarrier): Future[Option[RecentSubmissionDetails]] =
    httpClient.get(url"${appConfig.digitalPlatformReportingUrl}/digital-platform-reporting/recent-submissions")
      .execute[HttpResponse]
      .flatMap { response =>
        response.status match {
...
object PendingEnrolmentConnector {
  ...
  final case class SaveRecentSubmissionFailure(status: Int) extends Throwable {
    override def getMessage: String = s"Save recent submission failed with status: $status"
  }
}