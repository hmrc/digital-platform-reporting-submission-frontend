...
case class CreateAssumedReportingSubmissionRequest(operatorId: String,
...
object CreateAssumedReportingSubmissionRequest {

  def build(answers: UserAnswers): EitherNec[Query, CreateAssumedReportingSubmissionRequest] =
    (
      getAssumingOperator(answers),
      answers.getEither(ReportingPeriodPage)
    ).parMapN { (assumingOperator, reportingPeriod) =>
      CreateAssumedReportingSubmissionRequest(
